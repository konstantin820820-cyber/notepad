...
class _HomeScreenState extends State<HomeScreen>
  ...
  void _createTabController(int selectedIndex) {
  ...
  void _recreateTabController(int selectedIndex) {
    _createTabController(selectedIndex);
  ...
  Future<void> _renameCategory(String category) async {
    ...
    _recreateTabController(selectedIndex);
  ...
  Future<void> _deleteNote(Note note) async {
    ...
    setState(() {
      ...
      _trash.removeWhere((n) => n.id == note.id);
  ...
  Future<void> _restoreFromTrash(Note note) async {
    ...
    _showMessage('Заметка восстановлена');
  }
...
class _EditorScreenState
  ...
  FleatherController _createController() {
    if (widget.note != null &&
        widget.note!.contentJson.trim().isNotEmpty) {
      try {
        final decoded =
            jsonDecode(
  ...
  // TOOLBAR
  //
  // Fleather 1.27.0 штатно поддерживает:
  // bold
  // italic
  // underline
  // strike
  ...
  Widget _buildToolbar() {
    return FleatherToolbar.basic(
      ...
      hideListNumbers: false,

      hideListBullets: false,

      hideListChecks: false,
      ...
      hideForegroundColor: false,

      hideBoldButton: false,

      hideItalicButton: false,

      hideUnderLineButton: false,

      hideStrikeThrough: false,
